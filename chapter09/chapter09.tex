...
\begin{feature}
  ...
  In Python, most functions are built-in, but will show how to use these functions in \index{pandas}\emph{pandas} and also introduce the \index{regex}\emph{regex} package, an alternative to built-in regular expressions.
...
you either need to use one of /textit{Pandas}' methods shown below or use a \index{for loop}\emph{for loop} or \index{list comprehension}\emph{list comprehension} to iterate over all the strings (see also section 3.2).
...
%%   However, for almost all examples you can also use the \index{re}\emph{re} module if preferred.
%%   }
...
The \index{re}\emph{re} package is more common and significantly faster, but does not support Unicode character properties (\verb|\p|).
...
One thing to note in the previous example is the use of the index \verb|[[1]]| in R to select the first element in a list.
