...
\pyrex[input=py, output=py,caption=Crossvalidation]{chapter09/crossval}
...
bootstrapping. \refex{gridsearch}, \refex{gridsearch2}, and
\refex{gridsearch3} illustrates how you can automatically assess which
...
%\pyrex[input=r,output=r,caption="A gridsearch in R. Note that in R, you cannot (at least, not easily) put any parameter of the model in the grid. Instead, you can look up the ``tunable parameters'' which \emph{must} be present part of the grid in the caret documentation. This means that an exact replication of the grid searches in \refex{gridsearch} and \refex{gridsearch2} is not natively supported using \pkg{caret} and requires either manual testing or writing a so-called caret extension." ]{chapter09/gridsearch3}
