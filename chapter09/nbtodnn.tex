...
media use, and so on.  And in textual data, whether a word X is used
is not independent from the use of word Y --- after all, both are not
...
\pyrex[output=py, caption=A simple Na\"ive Bayes classifier]{chapter09/nb}

Example~\refex{nb} illustrates how to do train a Na\"ive Bayes
...
\pyrex[output=py, caption=A simple logistic regression classifier]{chapter09/logreg}
...
\todo[inline]{maybe more detail/math/plots needed} Support Vector
...
function (RBF), or Gaussian kernels.  \todo[inline]{decide how far we
  want to go here}.

\pyrex[output=py, caption=A simple Support Vector Machine classifier]{chapter09/svm}
...
\pyrex[output=py, caption=A simple Random Forest classifier]{chapter09/randomforest}
...
In all of the models we discussed so far (which are also referred to as ``classiical machine learning''), we assume a direct relationship between the features and the labels. Grapically, we could depict this as direct arrows from all features to the output we want to predict (Figure~\ref{fig:classicalml}).
