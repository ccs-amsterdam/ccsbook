...
\todo[inline]{review R code examples, decide on which libraries to use (there seems no agreed-on consensus such as scikit-learn?)}
\label{sec:nb2dnn}
...
The Bayes part of the Na\"ive Bayes classifier comes from the fact that it uses Bayes' formula, $$ P(A \mid B) = \frac{P(B \mid A) \cdot P(A)}{P(B)} $$.
...
\rpyex{chapter09/snippets/nb}{A simple Na\"ive Bayes classifier}

Example~\refex{nb} illustrates how to do train a Na\"ive Bayes classifier.
...
\rpyex{chapter09/snippets/logreg}{A simple logistic regression classifier}
...
\rpyex{chapter09/snippets/svm}{A simple Support Vector Machine classifier}
...
\rpyex{chapter09/snippets/randomforest}{A simple Random Forest classifier}
