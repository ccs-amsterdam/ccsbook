...
It represents a \concept{corpus} (or set of documents) as a matrix or table, where each row represents a document, each column represents a term (word),
and the numbers in each cell show how often that word occurs in that document.

\pyrex[caption=Example document-term matrix,output=r,format=table]{chapter11/dtm}
...
In R, you can use the \fn{dfm} function from the \pkg{quanteda} package \citet{quanteda}.
...
\pyrex[caption=Differences between tokenizers]{chapter11/tokenize}
...
In the box ``Tokenizing in Python'' below, we therefore discuss some alternatives. For instance, the \cls{TreebankWordTokenizer} included in the \pkg{nltk} package is a more reasonable tokenizer and 
...
which splits text into sentences (using \fn{nltk.sent\_tokenize}) -- see the box for more details.
...
\begin{feature}\textbf{Tokenization in Python}\\
  ...
  Fortunately, there are other tokenizers in the \pkg{nltk.tokenize} package that do better.

  For example, the \fn{TreebankTokenizer} uses the tokenization rules for the Penn Treebank
  to tokenize, which produces better results:

  \codex[caption=The nltk.tokenize.TreebankTokenizer]{chapter11/tokenizealt.py}
  
  Another example is the \fn{WhitespaceTokenizer}, which simply uses whitespace to tokenize,
  which can be useful if your input has already been tokenized,
  and is used in \refex{tagcloud} below for tweets to conserve hash tags.

  \codex[caption=The nltk.tokenize.WhitespaceTokenizer]{chapter11/tokenizealt1.py}

  You can also write your own tokenizer if needed.
  For example, the \fn{TreebankTokenizer} assumes that text has already been split into sentences
  ...
  which uses \fn{nltk.sent\_tokenize} to first split the text into sentences,
  and then uses the \fn{TreebankTokenizer} to tokenize each sentence,
...
\pyrex[caption=Example document-term matrix]{chapter11/sotu}
...
\begin{ccsexample}
  \doublecodex{chapter11/freq}
    \doubleoutput{chapter11/freq}
    \doublecodex{chapter11/freq2}
      \doubleoutput{chapter11/freq}
...
In the \refsec{ngram} we will look at some ways to create a richer matrix-representation by using word pairs.
...
\pyrex[caption=Word Cloud of the US State of the Union corpus,format=png]{chapter11/wordcloud}
