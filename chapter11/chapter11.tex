...
\begin{feature}
  ...
  For supervised text analysis, we use \index{quanteda.textmodels}\emph{quanteda.textmodels} in R, and \index{sklearn}\emph{sklearn} and \index{keras}\emph{keras} in Python.
  For topic models we use \index{topicmodels}\emph{topicmodels} (R) and \index{gensim}\emph{gensim} (Python).
...
\verb|text_train| are the training texts and \verb|y_train| are the corresponding labels.
...
or SentiStrength \citep{Thelwall2012} include such functionalities. Yet, as we will
...
\subsection{Putting Together a Workflow}\label{sec:workflow}
...
representations that we know: word counts and \verb|tf.idf| scores
...
regression with a \verb|tf.idf| vectorizer. You could also try fitting
...
\subsection{Using the Model}\label{sec:usingmodel}
...
\begin{feature}
  ...
  please see \url{https://cssbook.net/lda} for additional material and visualizations.
\end{feature}

\subsection{Fitting an LDA Model}\label{sec:ldafit}
...
Next, we fit the actual LDA model using the package \index{gensim}\emph{gensim} (Python) and \index{topicmodels}\emph{topicmodels} (R).
...
specify \verb|alpha='auto'|, which will learn an asymmetric prior from the data. See the note on hyperparameters for more information.
...
\subsection{Validating and Inspecting Topic Models}\label{sec:ldavalidate}
...
\subsection{Beyond LDA}\label{sec:beyondlda}
...
These include, for example, \textit{Dynamic Topic Models} (which incorporate time; \citealp{dynamiclda}) and
\textit{Correlated Topic Models} (which explicitly model correlation between topics; \citealp{correlatedlda}).
...
Python users should check out \textit{Hierarchical Topic Modeling} \citep{hierarchicallda}.
...
\begin{feature}
  ...
  \index{Coherence}\emph{Coherence} is a measure of how semantically coherent the topics are by checking how often the top token co-occurs in documents in each topic \citep{mimno11}.
...
\begin{ccsexample}
  ...
  \caption{Computing perplexity and coherence of topic models}\label{ex:ldacoherence}
\end{ccsexample}
