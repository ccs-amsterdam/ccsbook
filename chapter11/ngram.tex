...
\pyrex[caption=Using UDPipe to analyse a sentence,format=table,output=r]{chapter11/udpipe}
...
It is written in Python, but the \pkg{spacyr} package offers an easy way to use it from R.
For R users, installation of \pkg{spacyr} on MacOS is easy,
...
\refex{spacy} shows how you can use Spacy to analyse the proverb `all roads lead to Rome' in Spanish.
...
Note that in R, you can use the \pkg{quanteda} function \fn{as.tokens} to directly use the Spacy output in quanteda.
...
Especially \concept{Stanza} \citep{stanza} is also very good and flexible toolkits with support for multiple (human) languages and good integration especially with Python.
