...
Generally, data with multiple observations of the same unit is called \concept{wide data} (as there are many columns),
...
\refex{pivot} shows how you can `pivot' the capital data to long format using \fn{pivot\_longer} (R) and \fn{melt} (Pandas). The second part of this example then goes on to do this for both data sets, merge them, and partially reproduce Figure~4.4 from \citet{piketty}.
...
\reffig{messy} shows a screenshot of this data in Libre Office, with the U.S. data having a similar shape.
...
Fortunately, these steps have been discussed before: reading csv data in \refsec{reading}; pivot to long data in \refsec{pivot};
add a column in \refsec{calculate}; joining data in \refsec{join}; and visualizing in \refsec{visualization}.
