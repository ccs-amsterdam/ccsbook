...
In the examples above, each line of code (often called a \emph{statement}) contained a single operation, generally a call to a function or method (see \refsec{functions}).
...
\refex{aggregate2} shows the same operation as in \refex{aggregate}, but chained into a single statement.
...
replacing the \pkg{dplyr} function \fn{summarize} by the function \fn{mutate} discussed above.
