...
\begin{ccsexample}
 ...
 I have changed them to double quotes, however, please check that I have not inadvertently changed any which genuinely should be single quotes.}  As you see in Example~\ref{ex:var4}, you can create a string by enclosing  text in quotation
...
from a vector or list by simply putting the index between square brackets |[]| (Example~\ref{ex:1darray2}). %In Example~\ref{ex:1darray2}, we
...
a range of numbers.%, from 1 to 20 or from -5 to 5 (Example~\ref{ex:1darray3}).
...
%elements by using the function \fn{length} (Example~\ref{ex:1darray3}).
...
%% Example~\ref{1darray6}.  The first line will create again a vector of scores
...
%% which we will use later throught this book, introduces a data type called
...
%Now that you have gone through most types of objects in R and Python, we need to remember that objects 
