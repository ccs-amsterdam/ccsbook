...
using the square brackets [] over the vector. In \refex{1darray2}, we
...
function. \refex{1darray3} illustrates how to add new values to an array
...
each value. Using the operator \fn{:} (R) or the function \fn{range} (Python), we can create numeric vectors with
...
In \refex{1darray5}, we create a list and a tuple, and get their sizes with the native function |len|
...
Finally, a data structure very similar to a vector is the \fn{factor}.
