...
Let's create an object that we call |a| (an arbitray name, you can use
...
Let's have a closer look at the code in \refex{var1} above.
...
with |a| and other values or new variables as shown in \refex{var2}. For example, you
...
you can \concept{escape} the quotation mark by prepending it with a backslash (\texttt{\textbackslash'}),
and similarly for double-quoted strings.
To include an actual backslash in a text, you also escape it with a backslash,
so you end up with a double backslash (\texttt{\textbackslash\textbackslash}). 
...
when we create a vector with for example numeric data, the \fn{class} function will display
...
You create a named list with \texttt{d = list(name=value)} and access individual elements with either
\ttt{d\$name} or \texttt{d[["name"]]}. 
...
Data frames are discussed in detail in \refchap{filetodata},
