...
script that you can then run partially or entirely. Recall \refsec{installing},
...
In \refex{listcomprehensions} we provide a simple example (without any
...
Suppose you want to print the headlines of \refex{forloop} only if the text is less than 40 characters long.
To do this, we can include the conditional statement in the loop, executing the body only if the condition is met (\refex{if1})
