...
\subsection{Na\"ive Bayes} \label{subsec:Na\"ive Bayes}
...
\begin{ccsexample}
  ...
  \caption{Choosing a different cutoff point for predictions with logistic regression. In this case, we make a trade-off and maximize the difference between false positive rate and true positive rate to improve the precision for the the second category the precision for the first category}
...
\subsection{Cross-validation and Grid Search} \label{sec:crossvalidation}
...
% WvA moved this from the caption as it is a bit long and messed up the html script
Note that in R, not all parameters are ``tunable'' using standard \index{caret}\emph{caret}. Therefore, an exact replication of the grid searches in Example~\ref{ex:gridsearch} and Example~\ref{ex:gridsearch2} would requires either manual comparisons or writing a so-called caret extension.
...
\pyrex[input=r,output=r,caption={A gridsearch in R. }]{chapter08/gridsearch3}
...
\begin{feature}
    ...
    Hopefully, the \pkg{tidymodels} package will provide a better solution for machine learning in R
    in the near future.
\end{feature}
