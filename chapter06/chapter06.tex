...
\begin{feature}
  ...
  This chapter relies mostly on the \pkg{pandas} (Python) and \pkg{tidyverse} (R) functionality to read and write files. Additionally, \pkg{haven} is used to read data from other tools such as SPSS.
  Finally, we show how to use existing data from packages such as \pkg{sotu} (R) and \pkg{nltk} and \sklearn\ (Pythn).
