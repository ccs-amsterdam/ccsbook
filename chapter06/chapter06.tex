...
\rpyex{chapter06/snippets/createdataframe}{Creating a dataframe from other datastructures}


In this book, we will use dataframes a lot, because they are very convenient for handling tabular data, and because they provide a lot of useful convenience functionality, instead of requiring us to re-invent the wheel all the time. In the next section, we will discuss some of them.
...
\todo[inline]{maybe also (somewhere in this chapter) say more about basic properties of data frames, such as there length, shape, ...}

\todo[inline]{also something about column names etc, indices....}
...
\rpyex{chapter06/snippets/readfiles}{Reading files into a dataframe}
...
\rpyex{chapter06/snippets/stopwords}{Reading files without dataframes}



\todo[inline]{We need to look at what the best strategy is here, especially as there are big differences between R and Python here. To get started, the stuff above should be more than fine, but I think we need to also explain the ``manual'' way below, especially because people might need to extract something from deeply nested json files or whatever later on (actually, I even test these kind of things in exams). But I dont even  know how this stuff is usually approached in R }


!!!!!!!!!!!!!PYTHON-specific stuff below !!!!
But let us use this example to get a bit deeper into the general way of handling files.

\rpyex{chapter06/snippets/stopwordsextensive}{Reading files without dataframes} 
...
For example, we could loop over all lines in the file and add it to a list
