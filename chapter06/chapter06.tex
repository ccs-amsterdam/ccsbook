...
%\input{chapter06/data_filtering}
...
\pyrex[caption={Aggregation. Note that in the Python example, we can specify often-used functions such as \texttt{"mean"} simply as a string, but instead, we could also pass functions directly, such as numpy's \texttt{np.mean}},output=r,format=table]{chapter07/aggregate}
...
\pyrex[caption={Combining multiple functions or methods. The result is identical to \refex{aggregate}},output=none,format=table]{chapter07/aggregate2}{
...
\pyrex[caption=Adding summary values to individual cases,output=r,format=table]{chapter07/transform}
...
%\pyrex[caption=Private and Public Capital data (source: Piketty 2014)]{chapter07/piketty}

\begin{ccsexample}
  \doublecodex{chapter07/piketty_1}
  \codexoutputtable{chapter07/piketty_1.r}
  \doublecodex{chapter07/piketty_2}
  \codexoutputtable{chapter07/piketty_2.r}
...
\begin{ccsexample}
\doublecodex{chapter07/capital_1}
\codexoutputtable{chapter07/capital_1.r}
\doublecodex{chapter07/capital_2}
\doubleoutput{chapter07/capital_2}
\caption{Merging private and public data for France.}\label{ex:merge}
\end{ccsexample}


%\pyrex[caption=Merging private and public data for France]{chapter07/capital}
...
\pyrex[caption={2016 Primary results and county-level metadata. Note that to avoid duplicate output, we display the counties data in the Python example and the results data in the R example.}]{chapter07/primary}
...
\pyrex[caption=Joining data at the result and the county level]{chapter07/nested}
...
In fact, when we merged these datasets earlier in Example~\ref{ex:merge}, we selected only the measurements for France, essentially turning it into long data.

\begin{ccsexample}
  \doublecodex{chapter07/merge1}
  \codexoutputtable{chapter07/merge1.r}
  \doublecodex{chapter07/merge2}
  \begin{tcolorbox}[title=Output]
    \includegraphics[width=.5\linewidth]{{snippets/chapter07/merge2.r}.png}
  \end{tcolorbox}
  \caption{Converting wide to long data to facilitate merging and visualizing.}\label{ex:merge}
\end{ccsexample}

\refex{pivot} shows how you can ``pivot'' the capital data to long format using \index{pivot\_longer}\texttt{pivot\_longer} (R) and \index{melt}\texttt{melt} (Pandas). The second part of this example then goes on to do this for both datasets, merge them, and partially reproduce Figure~4.4 from \citet{piketty}.
...
\begin{ccsexample}
  \doublecodex{chapter07/excel1}
  \codexoutputtable{chapter07/excel1.r}
  \doublecodex{chapter07/excel2}
  \begin{tcolorbox}[title=Output]
    \includegraphics[width=.5\linewidth]{{snippets/chapter07/excel2.r}.png}
  \end{tcolorbox}
