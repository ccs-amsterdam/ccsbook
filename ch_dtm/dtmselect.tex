...
\pyrex[caption=Top words used in Trump Tweets,format=table,output=py]{ch_dtm/trumptweets}
...
\pyrex[caption=Simple stop word removal,format=png,output=r]{ch_dtm/stopwords}
...
\begin{ccsexample}
\doublecodex{ch_dtm/noise1}
\codex[caption=Output]{ch_dtm/noise1.r.out}
\doublecodex{ch_dtm/noise2}
\codex[caption=Output]{ch_dtm/noise2.r.out}
\doublecodex{ch_dtm/noise3}
\codex[caption=Output]{ch_dtm/noise3.r.out}
...
\pyrex[caption=Cleaning the whole corpus and making a tag cloud,output=r,format=png]{ch_dtm/tagcloud}
...
In Python, you can see that we now have a nested list comprehension, where outer loop iterates over the texts and the inner loop iterates over the tokens in each text.
...
\pyrex[caption=Trimming a Document-Term Matrix]{ch_dtm/trimming}
...
\pyrex[caption=Tf-Idf weighting,output=r,format=table]{ch_dtm/tfidf}
