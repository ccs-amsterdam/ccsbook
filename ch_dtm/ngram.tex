...
This this case, we create a DTM of the state of the union matrix with all bigrams included.
...
For Python, we use the \pkg{gensim} package which we will also use for topic modeling in \refchap{lda}.
This package has a \cls{Phrases} class which can score the bigrams in a list of tokens.
...
After setting a threshold of 0.7, we can use \fn{fit\_transform} to change the tokens.
...
For example, one of the authors of this book use syntactic relations to analyse conflict coverage,
...
We would advice you to almost always consider lemmatizing and tagging your texts, as lemmatizing is simply so much better than stemming
