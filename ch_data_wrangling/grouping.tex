...
\pyrex[caption={Aggregation. Note that in the Python example, we can specify often-used functions such as \texttt{"mean"} simply as a string, but instead, we could also pass functions directly, such as numpy's \texttt{np.mean}},output=r,format=table]{ch_data_wrangling/aggregate}
...
\pyrex[caption=Combining multiple functions or methods,output=r,format=table]{ch_data_wrangling/aggregate2}{
