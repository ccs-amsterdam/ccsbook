...
you either need to use one of Panda's methods shown below or use a \concept{for loop} or \concept{list comprehension} to iterate over all the strings.
...
case letters.
Using the \pkg{stringr} function \fn{str\_replace\_all} in R
and \fn{replace} in Python we can do a find-and-replace and replace substrings
by others (in our case, replace \texttt{<b>} with a space, for instance).
To remove unnecessary double spaces we apply the \fn{str\_squish} function provided by \pkg{stringr} and in Python,
we first chunk our string into a list of words by using the \fn{split} string method, before we use the \fn{join} method
to join them again with now a single space.
In the case of converting letters from upper to lower case, we use the base R function \fn{tolower} and the string
method \fn{lower} in Python.
Finally, the base R function \fn{trimws} and the
Python string method \fn{strip} remove the white space from the beginning
and end of the string.
\refex{clean} shows how to conduct this cleaning process.

While you can get quite far with these techniques, there are more advanced and flexible approaches possible.
For instance, you probably do not want to list \emph{all possible} HTML tags in separate \fn{replace} methods or \fn{str\_replace\_all} functions. In the next section, we therefore show how to use so-called \concept{regular expressions} to formulate such generalizable patterns.

\pyrex[output=both,caption=Some basic text cleaning approaches]{chapter10/clean}

\note{\textbf{To regex or not to regex in Python} You may wonder why we introduce basic string methods like \fn{replace} or the \fn{split}-then-\fn{join} trick, if everything can be done with regular expressions anyway. There are a couple of reasons for still using these methods: First, they are easy and don't have any dependencies. If you just want to replace a single thing, then you don't need to import any additional module. Second, regular expressions are considerably slower than string methods -- in most cases, you won't notice, but if you do a lot of replacements (think in thousands per news article, for a million of articles), then this may matter. Third, you can use the \fn{join} trick also for other things like punctuation removal -- in this case, by generating a list of all characters in a string called \verb|text| provided they are no punctuation characters, and then joining them directly to each other:
