...
For instance, the \index{TreebankWordTokenizer}\texttt{TreebankWordTokenizer} included in the \index{nltk}\emph{nltk} package is a more reasonable tokenizer and
...
\begin{feature}
  \textbf{\index{lambda}\texttt{lambda} functions in Python.}
  ...
  Instead of defining a reusable function with the \index{def}\texttt{def} keyword and then to call it by its name when we need it later,
...
\subsection{Trimming a DTM}\label{sec:trimdtm}
...
\subsection{$n$-grams}\label{sec:ngrams}
...
These packages each use a different implementation: \index{gensim}\emph{gensim} uses pointwise mutual information,
...
For R users, installation of \index{spacyr}\emph{spacyr} on MacOS and Linux is easy,
